...
# the tsgolint PATH wrapper, and the --type-aware install check are inherited.
{
  ...
  rev = "1e902cc1adb7a37c146f9bcdaf891f81af33d8df";
  unpatchedSrc = ourPkgs.fetchFromGitHub {
    owner = "oxc-project";
    repo = "oxc";
    inherit rev;
    hash = "sha256-ji0UKO6YZHfWYoMqZKnAExPEgvO503iSaZsdZNJQbuc=";
  };
...
in
  (ourPkgs.oxlint.override {inherit tsgolint;}).overrideAttrs (finalAttrs: prev: {
    ...
    cargoDeps = ourPkgs.rustPlatform.fetchCargoVendor {
      ...
      hash = "sha256-sK3AaRjQcBrIOGZT5hrxJuD+08S8lKUa4jpC8OCUrXo=";
    };
    pnpmDeps = ourPkgs.fetchPnpmDeps {
      inherit (finalAttrs) pname version src;
      pnpm = ourPkgs.pnpm_11;
      fetcherVersion = 4;
      hash = "sha256-0saZZzHYi62dNffhclIfowbCpNJAaHS9+bcAl3bJjQ8=";
    };
