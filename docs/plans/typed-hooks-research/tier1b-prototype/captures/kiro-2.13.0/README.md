...
Real hook `stdin` payloads captured from a **live Kiro v3 TUI session** on 2026-07-20
(HITL probe, session `sess_a44bce46-…`). Each file is the exact JSON Kiro piped to a
hook for that trigger — primary-source evidence for the Tier-1b `capture→replay`
fixtures (see `../../README.md` and assessment §9/§12).

These are **raw captures, not fixtures.** A fixture wraps one of these payloads with
`expect` assertions + a hook-under-test (that authoring step is the groomed backlog —
do not build them all out here). `cwd` is spliced to `@CWD@` when a payload becomes a
side-channel fixture.
...
Two triggers documented in the assessment did **not** fire in this probe and remain
open (backlog): `Manual` (the `/remember` slash path — Q2) and the file-lifecycle set
(`PostFileSave` etc., `documentedAbsent` in the trigger sidecar).
