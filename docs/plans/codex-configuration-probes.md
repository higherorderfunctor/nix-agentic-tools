...
| `$CODEX_HOME/<name>.config.toml`                               | User         | `--profile` overlays this file above base config. Since 0.134.0, legacy `[profiles]` and top-level `profile` are unsupported.                                                                                                                                                                           | Declarative                                       | Static Nix-managed files: direct HM links or repository-declared devenv materialization into the native user lookup location.                                 |
