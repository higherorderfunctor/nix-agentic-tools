...
| Named profiles                                                  | `$CODEX_HOME/<name>.config.toml`, selected only by `--profile`; names allow letters, numbers, hyphens, and underscores                              | Expose as `ai.codex.profiles.<name>` in Home Manager   | Profiles are stable declarative TOML layers outside base `settings`. They have no native writer and are safe static files. Devenv rejects them because project config cannot select or host user profile layers.                                                                   |
...
## Profiles: intentional backend asymmetry
...
Home Manager therefore writes one static `${configDir}/<name>.config.toml` per
declared profile. The file is not part of the mutable base-config reconciliation
because no native writer shares it. Devenv exposes the same option tree for
discoverability but rejects a non-empty value: placing a profile beside project
`.codex/config.toml` would be inert, and changing CODEX_HOME would also fork
auth, sessions, logs, and caches.

This is semantic parity with an explicit scope limitation, not mechanical file
parity. There is deliberately no `defaultProfile` option because upstream no
longer supports a persistent selector; use `codex --profile <name>`.
