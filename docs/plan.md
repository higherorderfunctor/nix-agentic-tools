...
- [ ] **Hooks fail with stripped PATH on NixOS** — MEDIUM priority. Same class as the MCP `env` bug: project-level `PostToolUse` hooks like `prek run` fail with `/bin/sh: 1: prek: not found` because Claude Code spawns hooks with no PATH. Options: (a) update hooks to use absolute store paths, (b) document this in the shell-wrapper nix-standards fragment, (c) build a helper that runs a user command with a baked-in PATH from declarative inputs. Marketplace plugin hooks (hookify, ralph-loop, superpowers) also hit this when they use bare `python3`/`jq`.
- [ ] **Unify all MCP servers under ai.mcpServers / ai.\<ecosystem\>.mcpServers** — HIGH priority. End-state: consumers declare MCP servers exactly once in typed form; the factory translates per ecosystem. `ai.mcpServers` is the cross-ecosystem pool, `ai.<ecosystem>.mcpServers` is the per-ecosystem additive set (per-ecosystem wins on key conflict).
...
- [ ] **mcp-proxy auth metadata for kiro-cli 2.0** — kiro-cli 2.0 (bumped 2026-04-14, commit `557f3d9`) added a strict MCP authorization check on HTTP endpoints. Servers proxied via `mcp-proxy` (i.e., any server with `modes.http = "bridge"` in our schema — currently sympy-mcp) fail with `No authorization support detected`. Upstream sparfenyuk/mcp-proxy needs to ship auth metadata; until then the affected servers can't be reached by kiro-cli 2.0. Watch sparfenyuk/mcp-proxy releases. Workarounds for consumers: (a) pin kiro-cli back to 1.29.8, (b) run the affected server as stdio directly in kiro's mcp.json, (c) drop the bridge entirely.
