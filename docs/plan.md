...
- [x] **ai.copilot devenv project-scope restructure** — SHIPPED 2026-04-21. Added `ai.copilot.projectDir` (devenv-only option, default `.github`). Context → `.github/copilot-instructions.md`. Agents → `.github/agents/<name>.agent.md` (note new `.agent.md` suffix). Skills → `.github/skills/<skill>/*`. Rules unchanged (already at `.github/instructions/`). lsp-config / mcp-config / settings deliberately stay at `configDir` — Copilot has no project-scope reader for those; they exist to be pointed at by the wrapper flag. Design journal: `docs/ai-copilot-devenv-restructure-plan.md`.
