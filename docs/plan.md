...
- [ ] **Expose remaining upstream HM claude-code options on ai.claude** — upstream `programs.claude-code` has `plugins`, `marketplaces`, `agents`, `commands`, `rules`, `hooks`, `outputStyles`, `context`, `lspServers`. Claude-specific already routed through `ai.claude.*`: `plugins` (identity), `marketplaces` + `outputStyles` (shipped 2026-04-21). `context` via `ai.claude.context` / `ai.context`. `lspServers` shipped typed cross-ecosystem 2026-04-21 (e8b23ba). `agents` shipped cross-ecosystem Claude+Copilot 2026-04-21 (Kiro excluded — JSON shape differs). Remaining: `commands` (Claude-specific; may not cross-ecosystem cleanly), `hooks` (all three ecosystems differ significantly — Claude nested, Kiro JSON files, Copilot none).
