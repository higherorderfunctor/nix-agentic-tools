...
- [ ] **ai.claude.mcpServers schema mismatch** — HIGH priority. Factory declares `ai.claude.mcpServers` as an `attrsOf (submodule commonSchema)` with required typed fields (`type`/`package`/`command`/`args`/`env`/`settings`/`url`). But `lib.mkStdioEntry` returns freeform `{type, command, args, env}` matching upstream `programs.claude-code.mcpServers` schema. Consumer has to bypass `ai.claude.mcpServers` and write to `programs.claude-code.mcpServers` directly. Either add a freeform-passthrough option or redesign the typed schema to translate to the freeform output.
