...
> concern (mcpServers, context, rules, skills, agents, lspServers,
...
- [x] **Unified `ai.context` + `ai.rules` guidance surface** — SHIPPED across commits 8f0c16b, 7dad0b8, 419010a (2026-04-21), then extended to Codex in PRs #657, #658, and #668 (2026-08-01) and converted to typed content records with keyed matchers on 2026-08-15. `ai.context` + `ai.<cli>.context` cover Claude, Codex, Copilot, Kimchi, and Kiro with HM + devenv option parity; Copilot HM deliberately emits no project guidance. `ai.rules` + `ai.<cli>.rules` translate through the capable runtime transformers (Claude rules, Kiro steering, Copilot devenv instructions, and flat Codex AGENTS.md with explicit scope degradation and a 32 KiB default guard); Kimchi has no rules pool. The normalized `ai.instructions` surface is retired. Consumer configuration migrations remain different-repo concerns.
- [x] **Split normalized and native runtime settings** — SHIPPED 2026-08-15. Every runtime exposes the same closed `ai.<runtime>.settings` submodule for portable fields, while runtime-shaped passthrough and typed-native keys live under `ai.<runtime>.native.settings`. Claude's native JSON remains typed with a freeform tail and routes through the existing HM/devenv translators; the old freeform `ai.claude.settings` identity surface no longer exists.
  - *Correction note (2026-04-21):* earlier versions of this backlog listed a standalone "HM translation refactor" bullet. That was incorrect — HM already performed a trivial translation at the time. The eventual split made that translation explicit while preserving its native delivery behavior.
- [x] **Typed `ai.claude.plugins` schema** — SHIPPED as the attrset conversion. The original framing is kept for archaeology and is now obsolete: it said upstream HM accepted `listOf (either package path)`, that our option declared the same, and that the passthrough was therefore identity translation. Upstream has since moved to `either (attrsOf (either package path)) (listOf (either package path))`, deprecating and warning on the list form — a list derives each plugin's directory name from the entry's base name, so a bare flake-input store path yields an unstable `<hash>-source` that is renamed by every unrelated input bump. `ai.claude.plugins` is now `attrsOf (either package path)`, attrset-only with no list shim (deliberate breaking change), matching its `marketplaces` sibling; the attribute name is the plugin directory name. The passthrough applies `mkDefault` per ENTRY so a consumer overriding one plugin does not discard the rest. Opinionated per-plugin metadata is still not modelled and still not needed.
- [x] **ai.claude.memory migration** — OBSOLETE (2026-04-21). Upstream HM `programs.claude-code` renamed `memory` to `context` (confirmed: no `memory = mkOption` in current upstream source). Our factory uses typed `ai.claude.context` + `ai.context` records. Nothing to migrate; `text` and `source` definitions arbitrate by module priority, with same-priority definitions of both fields conflicting. Bullet kept for archaeology; absorbed by the unified guidance work.
- [x] **Expose remaining upstream HM claude-code options on ai.claude** — SHIPPED 2026-04-21. `ai.claude.*` surface now exposes all Claude-relevant upstream options: `plugins`, `marketplaces`, `outputStyles`, `native.settings`, `context`, `agents`, `commands`, `hooks`, `lspServers`. Current cross-ecosystem fanout preserves only real intersections: LSP reaches Claude/Copilot/Kiro (Codex has no native registry), semantic agents reach Claude/Codex/Copilot (Kiro's JSON model differs), and portable hooks span Claude/Codex lifecycle events. Commands remain Claude-only.
- [x] **ai.kiro.trustedMcpTools** — SHIPPED in commit f38d03f (`feat(kiro): add ai.kiro.tui + ai.kiro.trustedMcpTools options`). `ai.kiro.trustedMcpTools` (listOf str) and `ai.kiro.tui` (bool) expose the kiro-cli `--trust-tools` and `--tui` flags declaratively via the symlinkJoin wrapper. Consumer can drop the manual `kiro-cli-trusted` wrapper.
- [x] **ai.copilot devenv project-scope restructure** — SHIPPED 2026-04-21, with final parity hardening in CX-015. `ai.copilot.projectDir` defaults to `.github` and is declared identically in HM/devenv so both generated option trees remain discoverable and type-identical. Only devenv gives it project-relative semantics; HM keeps typed context/rule options for exact schema parity but deliberately emits neither, because github.com's reviewer consumes committed repository guidance. Devenv context, rules, agents, skills, and LSP servers honor `projectDir`, the last as `<projectDir>/lsp.json`, the repository-scope file Copilot reads; mcp-config / settings deliberately stay at `configDir` because Copilot has no project-scope reader for them.
...
- [x] **Unified `ai.mcpServers` + `ai.<cli>.mcpServers`** — SHIPPED across commits 24668ec + factory work, with Codex native TOML lowering in PR #676. Canonical instance of the transformer pattern for MCP servers. Typed schema at `lib/ai/mcpServer/commonSchema.nix` accepts 3 shapes (A: typed-via-package, B: raw command, C: HTTP). `lib.mkStdioEntry` returns the typed shape; `lib.renderServer` translates to ecosystem-native. Claude/Codex/Kiro/Copilot factories render merged servers to native paths, and Codex adds typed authentication, readiness, timeout, environment-name, and tool-approval extensions. Module-eval tests cover the pool. Consumer migrations are different-repo concerns.
  - **Historical blocker (resolved by the shipped translators).** `ai.claude.mcpServers` originally required a typed shape while `lib.mkStdioEntry` returned the freeform upstream shape, forcing consumers to bypass the shared pool; Kiro was hand-rolled too. The design and completed work items below record how that mismatch was removed.
  ...
  - **Work items.**
    - [x] Update `lib/ai/mcpServer/commonSchema.nix`: make `package` optional so the three shapes coexist; document discriminator rules (url → http, package → typed stdio, raw command → pass-through).
    - [x] Refactor `lib.mkStdioEntry` to return the typed schema shape instead of the current freeform output. Keep internal helpers intact.
    - [x] Add `renderServer` as a shared library helper OR as a method on each ecosystem factory. Landed as shared helper in `lib/mcp.nix`.
    - [x] `mkClaude.nix` HM config: map `mergedServers` through Claude's `renderServer` and write to `programs.claude-code.mcpServers` (top-level, not under settings).
    - [x] `mkKiro.nix` HM config: map `mergedServers` through Kiro's `renderServer` and write to `home.file."${cfg.configDir}/settings/mcp.json".text`.
    - [x] `mkCopilot.nix`: same pattern adapted to Copilot's surface.
    - [x] Update module-eval tests + any consumers of the schema.
    - [ ] Migrate nixos-config consumer — different-repo concern, tracked there.
...
- [x] **Extend `sharedOptions.nix` with `ai.lspServers` +
      ...
      is via `ai.claude.native.settings.env`, not a wrapper). Both now
      follow the `ai.skills` / `ai.rules` / `ai.mcpServers`
...
- [x] **HM ↔ devenv ai module parity test** — SHIPPED in CX-015. The generated
      option-reference check compares every flattened `ai.*` option name and
      type across the full HM/devenv module sets, not a brittle spot-check. It
      found and closed the sole declaration gap (`ai.copilot.projectDir`);
      backend-specific lifecycle behavior remains covered by module-eval tests
...
Shipped across PRs #653 through #688 (2026-08-01 through 2026-08-02). The
factory now covers both backends, native configuration ownership, shared
fanout, intentional exclusions, and a reverse gate against the pinned binary's
extracted surface. The checked items below retain the original milestone shape;
the detailed evidence lives in `docs/plans/codex-configuration-probes.md`,
`docs/plans/codex-native-surface-audit.md`, and
`docs/plans/codex-reverse-coverage-audit.md`.

- [x] Package chatgpt-codex CLI under `packages/chatgpt-codex/`
- [x] Add Codex AGENTS.md transformation through the shared `agentsmd`
      transformer
- [x] Wire the Codex factory into the module barrel via `collectFacet`
- [x] Add typed-plus-freeform settings, MCP, skills, semantic agents, hooks,
      sandbox/approval/permission policy, profiles, execpolicy, and exact
      extracted-surface coverage
