...
- [x] **ai.claude.memory migration** — OBSOLETE (2026-04-21). Upstream HM `programs.claude-code` renamed `memory` to `context` (confirmed: no `memory = mkOption` in current upstream source). Our factory uses `context` via `ai.claude.context` + `ai.context` (shipped in 8f0c16b). Nothing to migrate; consumers move to `ai.claude.context = ./path` or `ai.context = ./path`. Bullet kept for archaeology; absorbed by the unified instruction surface work.
...
- [x] **ai.copilot devenv project-scope restructure** — SHIPPED 2026-04-21, with final parity hardening in CX-015. `ai.copilot.projectDir` defaults to `.github` and is declared identically in HM/devenv so both generated option trees remain discoverable and type-identical. Only devenv can give the option project-relative semantics; HM rejects a non-default override with an actionable diagnostic instead of silently treating it as HOME-relative. Context, named instructions/rules, agents, and skills all honor that one root; CX-015 closed the original hardcoded-`.github/instructions` exception. lsp-config / mcp-config / settings deliberately stay at `configDir` — Copilot has no project-scope reader for those; they exist to be pointed at by the wrapper flag.
