...
- [ ] **Expose upstream HM claude-code options on ai.claude** — upstream `programs.claude-code` now has `plugins`, `marketplaces`, `agents`, `commands`, `rules`, `hooks`, `outputStyles`, `context`, `lspServers`. Our `mkClaude.nix` only delegates `enable`, `package`, `skills`, `settings`. Add declarations for options that don't have a cross-ecosystem concern yet (plugins, marketplaces, outputStyles — Claude-specific). Each is declared as our own typed option and routed to upstream via identity translation. For options that ARE cross-ecosystem (agents, commands, rules, hooks, context, lspServers), route through `ai.<concern>` + transformers instead. Replaces the bespoke `installClaudePlugins` activation script in nixos-config with declarative `plugins`/`marketplaces` options.
