...
- [x] **Unified `ai.context` + `ai.rules` instruction surface** — SHIPPED across commits 8f0c16b, 7dad0b8, 419010a, fab4e5c (2026-04-21). `ai.context` + `ai.<cli>.context` for Claude / Kiro / Copilot with HM + devenv parity; `ai.rules` attrs-shape + `ai.<cli>.rules` translating through existing per-ecosystem transformers (Claude rules, Kiro steering, Copilot instructions). `ai.<cli>.rules.<name>.sourcePath` added for out-of-store symlink live-edit (fab4e5c — the scope miss that was blocking consumer kiro-steering migration). Codex-side (mkCodex factory + 32 KiB size guard) deferred with the Codex ecosystem work. Consumer (nixos-config) kiro-steering migration is a different-repo concern. Full status in `docs/unified-instructions-design.md`; live-edit details in `docs/ai-rules-livelink-plan.md`.
...
- [x] **Expose remaining upstream HM claude-code options on ai.claude** — SHIPPED 2026-04-21. `ai.claude.*` surface now exposes all Claude-relevant upstream options: `plugins`, `marketplaces`, `outputStyles`, `settings`, `context`, `agents`, `commands`, `hooks`, `lspServers`. Cross-ecosystem fanout shipped where shapes converge: `lspServers` (typed, all three ecosystems, e8b23ba) and `agents` (Claude+Copilot, 048470b; Kiro excluded — JSON shape). Commands (41ef8eb) and hooks are Claude-only — Kiro uses JSON-shaped hooks on `ai.kiro.hooks`, Copilot has neither concept.
