...
class SourceContractTest(unittest.TestCase):
    ...
    def test_grammar_validation_has_positive_and_negative_controls(self) -> None:
        # POSITIVE control first: the same reply the negative case mutates must
        # pass, so a refusal below cannot be a validator that refuses anything.
        ...
        with self.assertRaisesRegex(ClientError, "invalid workspace.grammar result"):
            grammar_types(malformed)
        wrong_schema = json.loads(json.dumps(FIXTURE_GRAMMAR_REPLY))
        wrong_schema["schema"] = "scribe-grammar/2"
        with self.assertRaisesRegex(ClientError, "invalid workspace.grammar result"):
            grammar_types(wrong_schema)
