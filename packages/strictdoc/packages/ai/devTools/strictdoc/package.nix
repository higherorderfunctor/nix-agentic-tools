...
#
# Same contract as packages/semble/packages/ai/semble/package.nix: `//` preserves the upstream derivation's
...
#
# No consumer `pkgs` here, and no cache-hit-parity concern of the usual kind: the
...
# `strictdoc` row in packages/strictdoc/registry.nix still holds and is
...
# a `final`-bound wrapper around this package.
{
  ...
  strictdoc = inputs.strictdoc.packages.${pkgs.stdenv.hostPlatform.system}.default;
in
  strictdoc
  // {
    passthru =
