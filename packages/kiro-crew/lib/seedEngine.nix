...
# first-run download into a no-op. That is the entire mechanism.
{
  ...
  # ABSOLUTE STORE PATHS, not bare names on `runtimeInputs`' PATH.
  #
  # `runtimeInputs` does export a PATH covering these, and `nounset` makes a
  # PATH-less invocation fail loudly rather than silently resolving to the
  # working directory. But `checks/shell/bare-commands.nix` scans this tree
  # whole-line and cannot see that, and it is right to be conservative: the
  # context it defends is a wrapper spawned with a REPLACED environment, where
  # a bare name is a "command not found" at the worst moment. Absolute paths
  # cost a few characters and survive someone later editing `runtimeInputs`.
  # Spelled out rather than built by a helper. `checks/shell/bare-commands.nix`
  # excludes any line containing `/bin/`, and a helper that CONSTRUCTS that
  # substring leaves the literal off the line — so the binding names read as
  # bare commands to the scanner and it fails, correctly, on its own terms.
