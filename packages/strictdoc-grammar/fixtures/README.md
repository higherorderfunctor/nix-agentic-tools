...
grammar (`packages/tree-sitter-strictdoc/patches/tree-sitter-strictdoc.patch`)
cannot lex a `.` inside an unquoted `choice_option`, and
`checks/strictdoc/strictdoc-grammar-corpus.nix` requires that grammar to parse
every `.sdoc`/`.sgra` in this repository cleanly. Widening the charset to admit
one was MEASURED and rejected: it takes `strictdoc_04_release_notes.sdoc` from 9
error nodes to 89 and from 76795 error bytes to 98406, because tree-sitter lexes
by longest match over every token valid in the current state and a wider
`choice_option` out-competes the narrow ones. The QUOTED arm, which is what
actually buys the parenthesis this fixture exists to exercise, costs nothing and
was added in the same patch. A dot in an unquoted option is a tracked gap in the
editor grammar, not a surface feature this fixture ever claimed — see
