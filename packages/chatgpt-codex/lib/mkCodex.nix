# Codex-specific factory-of-factory.
{
  ...
  profileNameSafe = name: builtins.match "[A-Za-z0-9][A-Za-z0-9_-]*" name != null;
  mkProfileAssertions = profiles:
    ...
    ++ lib.concatLists (lib.mapAttrsToList (name: settings: [
        {
          assertion = profileNameSafe name;
  ...
  mkProfileEntries = prefix: profiles:
    lib.mapAttrs' (name: source:
      lib.nameValuePair "${prefix}/${name}.config.toml" {
        content = lib.mkDefault {inherit source;};
        executable = null;
      })
    # Keep invalid names out of the path-validated delivery map so the
    # profile-name assertion remains the diagnostic for the declaration.
    (mkProfileSources (lib.filterAttrs (name: _: profileNameSafe name) profiles));
...
in
  lib.ai.app.mkAiApp {
    ...
    config = {
      ...
      legacySettings =
        if isHm
        then cfg.nativeSettings
        else applyWorkspaceWriteRoots cfg.nativeSettings ["${config.devenv.root}/.git"];
    ...
    in
      lib.mkMerge [
        ...
        (lib.optionalAttrs isHm {
          ...
          ai.codex.files = lib.mkMerge [
            # Named profiles remain whole user-layer files, selected explicitly
            # by the CLI; they share neither the base config's native state nor
            # its leaf reconciliation ledger.
        ...
        (lib.optionalAttrs (!isHm) {
          ai = {
            # Profiles are discovered only in CODEX_HOME, so retain the guarded
            # host-directory materializer and its existing task identity.
            codex.activation.codexMaterializeProfiles = {
              after = [];
              command = "exec ${lib.getExe profileMaterializer}";
              entry.devenv = "ai:codex:materialize-profiles";
            };
