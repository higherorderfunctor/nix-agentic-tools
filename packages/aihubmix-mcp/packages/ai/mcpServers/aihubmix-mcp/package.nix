...
# `.override`-vs-`overrideAttrs` trap that packages/bruno/packages/ai/generic/bruno/package.nix
# documents for `lib.extendMkDerivation` builders simply does not arise —
# this is a 100%-delta fresh derivation, like packages/gluetun/packages/ai/generic/gluetun/package.nix.
...
# `the owner registry.nix` records the exclusion.
#
# TO BUMP (human, deliberate):
#   1. Re-author aihubmix-mcp-aihubmix-mcp-save-to-file.patch against the new
...
in
  buildNpmPackage {
    ...
    postPatch = ''
      cp ${../../../../src/aihubmix-mcp-package-lock.json} package-lock.json
    '';

    patches = [
      ../../../../patches/aihubmix-mcp-save-to-file.patch
    ];
