...
# (config-parity rule). The `ai.skills` pool is per-`evalModules`, so the HM
# module's contribution is invisible to devenv and vice-versa; each backend
# imports its own factory instance.
#
