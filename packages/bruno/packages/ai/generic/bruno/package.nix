...
# `buildNpmPackage`; wrap the BUILDER instead, the way lib/packaging.nix's
...
# argument-vs-`finalAttrs` trap packages/pnpm/lib/mkMajor.nix guards against.
...
# unwrapped.
{
  ...
  # owner-local source path is shared by both update stages.
