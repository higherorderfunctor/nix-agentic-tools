...
# in packages/claude-code/checks/module-eval.nix :: claude-hooks-settings-json-compose), so our hooks
...
# packages/claude-code/checks/claude-code-extracted.nix, which drift-guards the BINARY's hook-event
# set rather than devenv's module.
#
# HM's `programs.claude-code` is NOT a flake input, so its lowering is asserted
# from its documented type in lib/testing/module-harness.nix instead of here.
{
  pkgs,
  inputs,
  ...
