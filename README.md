...
| Portable reasoning effort | Per-CLI config | `ai.settings.reasoningEffort` (Claude + Codex + Copilot + Kimchi) | Same; Copilot's lands in `.github/copilot/settings.json`, Kimchi's in its project harness settings (see below). Kiro has only per-model native effort |
