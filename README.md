...
`modelRoles`, `multiModel`, `resources`, `shellProfileApiKeyMigrationDismissed`,
or `statusLine`. Set those with Home Manager, or configure them inside Kimchi so
...
the interactive prompt (the persisted decision applies to that path and its
descendants), setting `defaultProjectTrust = "always"` at user scope, or passing
`--approve`. The flag works for CLI and TUI runs, but not ACP: ACP resolves
trust again per session without the CLI override.
...
live under `ai.codex.settings` and narrow `ai.settings` field by field. The
named whole-file `ai.codex.profiles` layer was removed as unreachable — see the
sandbox section above for why. Native Starlark command policy uses
