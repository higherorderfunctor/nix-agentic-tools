{
  ...
  ownedDocument = runtime: path: evaluated:
    ...
    evaluated.config;
in {
  inherit aiBase aiStubs devenvStubs evalDevenv evalDevenvWithGetEnv evalDevenvWithSpecialArgs evalHm evalHmWithSpecialArgs harnessNames hasLiteral hmLib hmStubs mcpConfigKeyOf mcpLib mkAssertion mkTest mkWrapperGrepTest ownedDocument tomlFormat;
  inherit testing;
}
