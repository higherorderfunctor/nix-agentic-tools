...
open sibling origin KS, and an H chain of depth 1, the single node KD1, which is
closed. The chain has no interior node.

Change: create K0, KS and KD1, then add one R occurrence to KS targeting KD1 at
occurrenceIndex 1. created lists all 3 new uids.

Expected: R.all is satisfied. The visible-target leaf reports code
visible-target with path and walkedPath both ["KS", "K0", "KD1"] and boundary
...
(contract.md:307-310). Depth 1 has no interior departure at all, and the closed
endpoint KD1 is still reached because "`visit: always` permits arrival at a
closed node, including the endpoint" and "The final endpoint needs no expansion"
(contract.md:306-313).
