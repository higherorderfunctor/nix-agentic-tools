...
# Workspace-relative root of the SHARED synthetic queue, used by the
# drain-queue definition. Separate from STATE_ROOT because the two drains must
# be able to coexist in one workspace without either seeing the other's files.
readonly QUEUE_ROOT=".kiro-harness/queue"
...
# --- drain-queue -----------------------------------------------------------
#
# The SAME drain shape, pointed at the synthetic queue instead of at per-branch
# shard files. This is the definition design §6.4 actually specifies, and the
# difference from `drain.workflow.json` is the whole point rather than a detail:
#
#   drain.workflow.json   K branches, K INDEPENDENT shard files. Each branch
#                         owns its own work and can never contend. It measures
#                         that the engine refills branches independently.
#   this one             K branches, ONE shared queue. Every branch competes for
#                         every item, late-proposed work appears mid-run, and the
#                         claim has to be atomic or two branches do the same item.
#
# The first cannot exhibit the properties EXPECT.md's P1-P4 are about, because
# partitioned work has no contention to get wrong and no late arrivals to
# starve. It also cannot show a drain BEATING waves: with one shard per branch
# every branch does identical work, so the per-round maximum a wave pays equals
# the per-round average, and the two shapes cost the same. Measured on the first
# live run — 4.13x over serial, but only 3% over the wave counterfactual, which
# is the correct answer to a uniform workload and not a disappointing one.
# Heterogeneous item durations are what separate the two shapes, and only the
# queue has them (`severe` puts a 6 and a 10 behind eight 1s).
#
# EVERY BRANCH RUNS THE SAME PROMPT except for its owner label. That is
# deliberate: the branches are interchangeable claimants, so a branch is not
# assigned work, it goes and gets some. The owner label exists only so the queue
# can attribute a claim; nothing routes by it.
#
# THE STEP DOES ONE COMMAND AND STOPS. `queue_worker.py` is not a loop by
# construction (an empty claim RETURNS, it never polls), so one invocation is one
# item and the re-dispatch is the `repeat` node's job. That is also what keeps
# every worker short, which the compaction constraint requires.
#
# The command is emitted with the path QUOTED. `scripts_dir` is absolute and
# operator-supplied, so it can contain a space -- routine on macOS -- and the
# prompt instructs the agent to run the command EXACTLY as written, so an
# unquoted path is a word-splitting bug the model would faithfully reproduce.
# Fixing it here rather than in the JSON is the point of generating these: one
# edit covers all K branches, which is also how it went wrong in all K at once.
#
# `scripts_dir` is an INPUT rather than a baked path because it is absolute and
# machine-specific; a committed definition carrying one would be wrong on every
# other machine. The fileCheck path stays a LITERAL — see contract.jq's TRAP 3
# header: a path still holding a template marker after substitution skips
# containment validation entirely.
emit drain-queue -n \
  --argjson k "$K" \
  --argjson iters "$MAX_ITERATIONS_PER_BRANCH" \
  --arg agent "$WORKFLOW_AGENT" \
  --arg model "$STEP_MODEL_ID" \
  --arg effort "$STEP_EFFORT_LEVEL" \
  --arg queue_root "$QUEUE_ROOT" '
  def shard_label: if . < 10 then "0" + tostring else tostring end;

  {
    name: "mode-f-drain-queue",
    description: (
      "Drains ONE shared synthetic queue with \($k) interchangeable claimants: "
      + "a `parallel` over \($k) self-draining `repeat` branches, each branch one "
      + "`step` that invokes the queue worker exactly once. Unlike the shard "
      + "drain the branches CONTEND — every branch may claim any item — so this "
      + "is the definition that exercises atomic claiming, late-proposed work "
      + "and heterogeneous item durations. Terminates on the queue-published "
      + "`drained` flag, which is only true once ready, proposed, claimed and "
      + "orphaned are ALL zero. Generated by generate.sh — edit K there."
    ),
    inputs: {
      scripts_dir: "Absolute path to the harness scripts/ directory"
    },
    steps: [
      {
        type: "parallel",
        ...
        branches: [
          ...
          | {
              ...
              steps: [
                {
                  type: "step",
                  id: "branch-\($label)-claim",
                  agent: $agent,
                  modelId: $model,
                  effortLevel: $effort,
                  prompt: (
                    "Run exactly this one command, then stop:\n\n"
                    + "python3 \"{{scripts_dir}}/queue_worker.py\" "
                    + "--root \"\($queue_root)\" --owner branch-\($label)\n\n"
                    + "Then reply with exactly one line: exit=<the exit code of that command>\n\n"
                    + "Do NOT run the command more than once. Do NOT run any other "
                    + "command. Do NOT read, write or edit any file yourself — the "
                    + "command does all of that, and a hand edit corrupts a queue that "
                    + "\($k - 1) other branches are claiming from concurrently.\n\n"
                    + "Exit code 0 means one item was worked. Exit code 3 means the "
                    + "claim was empty; that is a NORMAL result, not an error, and you "
                    + "must still report it and stop rather than retrying."
                  )
                }
              ],
              maxIterations: $iters,
