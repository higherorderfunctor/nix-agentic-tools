...
#   kiroFiles   — { "filename.md" = content; } for Kiro steering files
{
  ...
  readmeMd = ''
    ...
    For Codex's legacy `workspace-write` sandbox, the module automatically adds
    the Nix cache and, under devenv, the current repository's `.git`. Integration
    ...
    Each feature and runtime set can be selected independently. Nullable
    feature values inherit the umbrella setting:
