...
#   kiroFiles   — { "filename.md" = content; } for Kiro steering files
{
  ...
  readmeMd = ''
    ...
    **yours**. Two consequences, and the second is the one that surprises
    ...
    Kimchi resolves project settings, MCP servers, and harness settings only
    under its exact working directory. When devenv delivers any of those files,
    its wrapper rejects launches below the devenv root instead of silently
    ignoring them. It does not change directories because that would also
    change the working directory seen by Kimchi's tools. Context and skills
    themselves walk ancestors, but the typed Kimchi settings include a default
    `skillPaths = []`, so every enabled devenv Kimchi currently delivers project
