...
def test_extensionless_routing() -> None:
    ...
    reader = GlobRoutingReader(routes=routes, path_root=str(REPO_ROOT))
    target = REPO_ROOT / "packages/claude-code/checks/fixtures/claude-hooks/post-edit"
    info = reader.read_from_file(str(target))
    assert info.file_stats.lines_total > 0, f"{target} was not read"
    assert ROUTING_STATS["packages/claude-code/checks/fixtures/claude-hooks/post-edit"] == 1, dict(
        ROUTING_STATS
    )
