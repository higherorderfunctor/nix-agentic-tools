...
def check_value_delimiters(field: str, value: str) -> None:
    ...
    A multiline field is written between `>>>` and `<<<`, and the closing
    marker is matched at column zero, so a value containing such a line
    terminates its own block early. validate_node does not see this -- the
    node is well-formed in memory and only the file is broken.
    """
    for lineno, line in enumerate(value.splitlines(), start=1):
        if line.startswith("<<<") or line.startswith(">>>"):
            raise SdocError(
                f"{field}: line {lineno} begins at column zero with "
                f"{line[:3]!r}, which would close or open a block and make "
                f"the written file unparsable. Indent the line by one space."
            )
...
class Graph:
    ...
    def _relation_type_for_role(element: GrammarElement, role: str) -> str:
        ...
        raise SdocError(
            f"{element.tag} may not make a {role!r} relation. "
    ...
    def remove_node(self, uid: str) -> Path:
        ...
        remaining = [
            ...
            if isinstance(other, SDocNode) and other.node_type != "TEXT"
        ]
        if remaining:
            raise SdocError(
                f"{path} holds {len(remaining) + 1} nodes; this tool writes and "
