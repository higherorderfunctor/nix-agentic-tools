  ...
  tasks = {
    "check:model-staleness" = {
      ...
      exec = ''
        ...
        # packages/claude-code/extracted.json, so they are covered by the
    ...
    # is wired into `nix flake check` as `packages/strictdoc-grammar/checks/` and `checks/strictdoc/`, which
    ...
    # `packages/strictdoc-grammar/checks/strictdoc-grammar-surface-live.nix`, and a second copy of that
        ...
        render() {
          nix eval --raw --impure --expr "
            let lib = import ${nixpkgsPath}/lib;
                g = import ./packages/strictdoc-grammar/lib/grammar.nix {inherit lib;};
