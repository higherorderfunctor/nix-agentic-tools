# Probe transcripts: devenv eval cache, direnv watch list, `devenv hook`

Raw evidence for the shell-activation architecture fragment
(`dev/fragments/shell-activation/activation-mechanism.md`), which cites this
file. It records what devenv's eval cache, direnv's watch list and `devenv hook`
each notice, and it is why this repository keeps `.envrc` instead of migrating
to `devenv hook`.

- **Measured:** 2026-09-22, one machine, `x86_64-linux`.
- **Versions:** devenv `2.3.2+87edd16`, direnv `2.37.1`. No pass recorded the
  Nix version.
- **Shape:** five agents over four measurement passes, appended in order — an
  initial eval-cache probe, a refutation of it, a direnv measurement pass, a
  refutation of that, and a final pass on what `devenv hook` watches. Later
  passes correct earlier ones.
- **Status:** a raw transcript, not curated prose. It was not line-edited and
  its claims were not individually reviewed. A sentence in an early pass may be
  overturned a few hundred lines later, so read a section together with the
  passes that follow it rather than quoting one in isolation. The reviewed
  conclusions are the fragment's; this file exists so they can be audited.
...
It previously lived only on the `archive/devenv-storecopy-probe` branch. It is
committed here because a citation on `main` must not depend on a ref that can be
deleted.
...
Worktree: `<worktrees>/devenv-storecopy` Branch: `probe/devenv-storecopy` (off
`origin/main` @ `8e4ed690`) Date: 2026-09-22 devenv:
`devenv 2.3.2+87edd16 (x86_64-linux)`
...
```
whitelist.prefix [<home>/Documents/projects <home>/Documents/work]
...
`whitelist.prefix [<home>/Documents/projects …]`, `Found RC allowed 0`, and no
allow file was created by either pass.
...
Home-manager config, read only (`home/<user>/global/default.nix`): `devenv`
