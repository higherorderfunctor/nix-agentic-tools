...
extractor runs that launch only in a Nix build sandbox with fake KAS and
isolated state, checks the materialized SHA-256, then parses the actual source.
HOME/XDG isolation alone is insufficient: native credential discovery can reach
host facilities outside those directories.
