...
> `enable = false`. Delivery diagnostics stay silent for excluded root pools
> while reporting unsupported per-runtime requests. `migrationConfig` retires
> files through an empty `own` target.
>
