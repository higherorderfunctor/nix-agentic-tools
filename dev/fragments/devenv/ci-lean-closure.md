...
> and never installed at all on devenv; `kimchi` is newly enabled and adds 154.3
> MiB (161,764,840 bytes) of CLOSURE to every shell, CI included. Both are
...
The weight is real and worth stating rather than discovering, and it is CLOSURE
weight — not binary size. Measured at kimchi 1.0.10, the version
`overlays/kimchi-sources.json` currently pins:

| figure               | bytes           |                                   |
| -------------------- | --------------- | --------------------------------- |
| `bin/kimchi` alone   | 116,974,792     | what a naive `stat` reports       |
| **realized closure** | **161,764,840** | **what the shell actually gains** |

Quote the closure figure. An earlier revision of this fragment cited the binary
size as the shell cost and understated the real number by ~45 MB — the exact
conflation a closure-taxonomy document exists to prevent. Re-measure on a bump
rather than scaling the old number:

```bash
nix path-info --closure-size -h "$(nix build --no-link --print-out-paths .#kimchi)"
```
