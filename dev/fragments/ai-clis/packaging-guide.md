...
> **Last verified:** 2026-09-14 — Kimchi builds its CLI and proxy helper from
> the release source; native shard exclusions and input-worker cache filters
> protect patched variants.
...
- `kimchi` — release `sources.json` + `ghArchiveUpdateScript`; derive the Go
  floor before the proxy-helper vendor hash, then refresh pnpm dependencies
