...
> **Last verified:** 2026-09-01 (commit pending — the `extraExtract` chain of
> every sidecar-pinned Go package is now GENERATED by `vu.mkGoUpdateExtract`
> rather than hand-composed. All seven chains had the vendor fixer before the
> floor fixer, which compiles Go under a toolchain the floor selects and the
> floor is ABSENT at that point, not stale — glab and oh-my-posh were held back
> on every sweep for it. `mkGoSrcVendorFix` is deleted; the order is gated by
