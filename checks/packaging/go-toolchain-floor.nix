...
# `vu.mkGoUpdateExtract` and `checks/packaging/go-floor-extract-order.nix`.)
#
...
#      vacuously true.
{
  ...
  in {
    ...
    go-toolchain-floor-satisfied-far-below =
      mkTest "satisfied-far-below" (
        (resolve {
          floor = "1.17";
          inherit ourGo;
        })
    ...
    go-toolchain-floor-satisfied-exactly =
      mkTest "satisfied-exactly" (
        (resolve {
          floor = ourGo.version;
          inherit ourGo;
        })
