...
- The two payloads differ in exactly one byte. A walk that skipped binary files
  instead of hashing them would collide the two trees.
...
- `symlinks/dangling` points at nothing on purpose. Resolving it aborts
  evaluation, so the walk has to skip symlinks rather than hash them.
